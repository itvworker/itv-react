...
export default class Scroller extends React.Component<ScrollerProps, ScrollerState> {
     ...
     */
    refresh = () =>{
        initClass.refresh.call(this);
    }
    /**
     * 
     * @param value {Boolean} 是否没有更多了
     */
    infinite = (value)=> {
     ...
     */
    get isVertcialMove() {
        return (this.props.pattern === 'vertical' || this.props.pattern === 'auto') && this.cache.direction === 'vertical'
    }
    ...
    get isHorizontalMove() {
        return (this.props.pattern === 'horizontal' || this.props.pattern === 'auto') && this.cache.direction === 'horizontal'
    }
    get scrollbarHeight() {
      return parseInt((this.state.contentHeight /this.state.maxY)*100);
    }
    get scrollbarWidth() {
        return parseInt((this.contentWidth / this.cache.maxX)*100);
    }
    get scrollBarOuter() {
    ...
    scrollY(n, o) {
        ...
        if(this.cache.isTriggerPullDown) {
            ...
            this.cache.text = this.props.refreshText;
        ...
        if(this.cache.isTouch && n < this.cache.pullDownPoint) {
            ...
            this.cache.text = this.props.loseenText
            this.cache.status = 1
        ...
        if(this.cache.isTouch && n > this.cache.pullDownPoint) {
            this.cache.text = this.props.pullText;
            this.cache.status = 0
            return
        }
    ...
    /**
     * 
     * @param obj {Object} x,y的值
     */
    emitScroll(obj) {
        Object.keys(this.events).forEach(item => {
            if(this.events[item] &&  this.events[item].scroll) {
                this.events[item].scroll(obj)
            }
        });
    ...
    emitInit = () => {
        Object.keys(this.events).forEach(item => {
            if(this.events[item] &&  this.events[item].init) {
                this.events[item].init();
            }
        });
    }
    ...
    render() {
        ...
        return (
            <ScrollerContext.Provider value={this}>
                <div ref={this.root}  className="itv-scroll" onTouchStart={(e) => this.touchstart(e, true)} onTouchMove={(e) => this.touchmove(e, true)} onTouchEnd={(e) => this.touchend(e, true)} onTouchCancel={(e) => this.touchend(e, true)} >
                    <div className="itv-scroll-content"  >
                        {scrollX}
                        {scrollY}
                        <div className="itv-scroll-touch" ref={this.scroller}  style={{'transform':`translate3d(-${this.cache.x},-${this.cache.y},0)`,'WebkitTransform':`translate3d(-${this.cache.x},-${this.cache.y},0)`}}>
                            ...
                            <div className={this.props.isMore && this.state.moreStatus!=="loadingStop"?'itv-scroller-more':'itv-scroller-more none' } ref={this.more} >
                                ...
                                <span  className={this.state.moreStatus === 'none'?'': 'none'}>{this.state.noDataText}</span>
                                <span className={this.state.moreStatus !== 'none'?'': 'none'}>{this.state.loadingText}</span>
                            </div> 
                        </div>
                    </div>
                </div>
